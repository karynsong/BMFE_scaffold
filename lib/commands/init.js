...
var schema = {
    properties: {
        ...
        platform: {
            ...
            message: '只能输入1和2',
            required: false,
            description: '请输入可初始化项目类型的序号：1. 正常项目 2.weex-eros',
            default: '1'
        }
    }
};
...
var platform_detail  = {
    '1': {
        name: 'nomal',
        handler: createFrameProject
    },
    '2': {
        name: 'weex-eros',
        handler: createWeexErosProject
    }
}
function helpTitle() {
    print.title(config)
    ...
    fse.ensureDir(projectPath, function() {
        ...
        child_process.exec(execString, function(error) {
            if (error !== null) {
                ...
                changeFile(projectPath + '/config.js', 'frameName', frame);
...
function createWeexErosProject(configJson, projectPath) {
    var frame = (argv.f || argv.frame),
        fullFrame = 'bmfe-' + frame;
    fse.ensureDir(projectPath, function() {
        var execString = 'cd ' + projectPath + ' && cnpm install ' + fullFrame +
            '&& cp -r ./node_modules/' + fullFrame + '/template/ ./ && rm -rf ./node_modules'
        child_process.exec(execString, function(error) {
            if (error !== null) {
                print.info('初始化失败: ' + error);
            } else {
                ...
                changeFile(projectPath + '/config.js', 'frameName', frame);
                fs.rename(projectPath + '/gitignore', projectPath + '/.gitignore');
                child_process.exec('cd ' + projectPath + '/fe' + ' && cnpm install', function(error) {
                    if (error !== null) {
                        print.info('初始化失败: ' + error);
                    } else {
                        print.info('初始化成功');
...
function createProject() {
    ...
    return prompt.get(schema, function(err, result) {
        ...
        configJson.platform = platform_detail[result.platform]['name'];
        projectPath = path.join(process.cwd(), configJson.name);
        if (argv.f || argv.frame) {
            platform_detail[result.platform]['handler'](configJson, projectPath);
        } else {
